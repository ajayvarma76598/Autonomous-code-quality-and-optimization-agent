...
class QueryRequest(BaseModel):
    ...
    snapshot_id: UUID | None = Field(
        default=None,
        description="Optional target snapshot ID. If omitted, uses the latest snapshot.",
    )
...
class CitationItem(BaseModel):
    provider: str | None = Field(
        ...
        description="Provider or service that supplied the citation.",
    )
    file_path: str
...
class QueryResponse(BaseModel):
    ...
    tools_used: list[str] = Field(
        ...
        description="List of internal tools, services, and evidence providers invoked for this query.",
    )
...
def extract_citations(state_out: dict) -> list[CitationItem]:
    ...
    def add_citation(
        file_path: str,
        provider: str = "VectorProvider / HybridSearch",
        module: str | None = None,
        function: str | None = None,
        snippet: str | None = None,
        score: float | None = None,
        ...
        clean_function = (
            str(function).strip() if function and function != "None" else None
        )
        ...
        key = (
            ...
            clean_module,
            clean_function,
            clean_snippet[:50] if clean_snippet else "",
        )
    ...
    for item in contexts:
        file_path = (
            getattr(item, "repository_path", None)
            or (item.get("repository_path") if isinstance(item, dict) else None)
            or getattr(item, "file_path", None)
            or (item.get("file_path") if isinstance(item, dict) else None)
        )
        module = getattr(item, "module", None) or (
            item.get("module") if isinstance(item, dict) else None
        )
        ...
        provider = getattr(item, "provider_name", None) or (
            item.get("provider_name")
            if isinstance(item, dict)
            else "VectorProvider / HybridSearch"
        )
    ...
    for msg in all_messages:
        ...
        if (
            msg_type == "tool"
            or "RetrievedContext" in content_str
            or "repository_path" in content_str
            or "chunk_id" in content_str
        ):
            items = []
            try:
            ...
            context_blocks = re.findall(
                r"RetrievedContext\((.*?)\)", content_str, re.DOTALL
            )
            ...
            paths_found = re.findall(
                r"['\"]repository_path['\"]:\s*['\"]([^'\"]+)['\"]", content_str
            )
...
def extract_tools_used(state_out: dict, citations: list[CitationItem]) -> list[str]:
    ...
    eval_data = (
        state_out.get("analysis", {}).get("evaluation", {})
        if isinstance(state_out.get("analysis", {}), dict)
        else {}
    )
    if eval_data:
    ...
    all_messages = state_out.get("shared", {}).get("messages", []) or state_out.get(
        "messages", []
    )
    all_messages = all_messages[msg_start:]
    for msg in all_messages:
...
def execute_query(request: QueryRequest, db: Session = Depends(get_db)) -> Any:
    """
    ...
    fast_cached_res = cache_service.check_exact_cache(
        str(request.session_id), safe_query
    )
    if fast_cached_res:
        ...
        return QueryResponse(
            ...
            citations=[
                ...
                for c in fast_cached_res.get("citations", [])
            ],
    ...
    if query_embedding:
        ...
        if cached_res:
            ...
            cache_tools = [
                "IntentClassifier",
                "VectorProvider (pgvector Hybrid Search)",
                "RedisCacheService",
            ]
            return QueryResponse(
                ...
                citations=[
                    CitationItem(**c) if isinstance(c, dict) else c
                    for c in cached_res.get("citations", [])
                ],
                tools_used=cache_tools,
                metadata=meta,
            )
    ...
    if not final_text or final_text == "No response generated.":
        ...
        if isinstance(analysis_dict, dict):
            for agent_key in [
                ...
                if agent_key in analysis_dict and isinstance(
                    analysis_dict[agent_key], dict
                ):
    ...
    grounding_score = float(
        eval_metrics.get("faithfulness", eval_metrics.get("confidence", 0.0))
    )
    ...
    msgs = state_out.get("shared", {}).get("messages", []) or state_out.get(
        "messages", []
    )
    msgs = msgs[msg_start:]
    input_tokens = sum(
        (getattr(m, "usage_metadata", None) or {}).get("input_tokens", 0)
        or (getattr(m, "response_metadata", None) or {})
        .get("token_usage", {})
        .get("prompt_tokens", 0)
        or (getattr(m, "additional_kwargs", None) or {})
        .get("usage", {})
        .get("prompt_tokens", 0)
        for m in msgs
    )
    ...
    try:
        ...
        query_record = QueryHistory(
            ...
            assistant_response=state_out.get("final_response")
...
async def execute_query_stream(
    ...
    try:
        ...
        # 1. Check Exact Cache
        cached_result = cache_service.check_exact_cache(
            str(request.session_id), safe_query
        )
    ...
    async def stream_generator():
        ...
        try:
            async for output in workflow_router.workflow.astream(
                state_in, config=config
            ):
                for node_name, node_state in output.items():
                    ...
                    payload = {
                        ...
                        "is_final": wf.get("status") == "COMPLETE"
                        or (
                            node_name in ["evaluation", "human_validation"]
                            and not wf.get("requires_escalation", False)
                        ),
                    ...
                    if payload["is_final"]:
                        ...
                        payload["tools_used"] = extract_tools_used(
            ...
            if final_state_out and query_embedding:
                try:
                    ...
                    cache_service.save_to_cache(
                        ...
                        query_text=safe_query,
                        query_embedding=query_embedding,
                        response=final_state_out.get("final_response")
                        or "No response generated.",
                        metadata={},
                    )
            ...
            if final_state_out:
                try:
                    ...
                    eval_metrics = final_state_out.get("analysis", {}).get(
                        "evaluation", {}
                    )
                    confidence_score = (
                        eval_metrics.get("confidence_score", 0.95)
                    ...
                    query_record = QueryHistory(
                        session_id=request.session_id,
                        user_query=request.query,
                        assistant_response=final_state_out.get("final_response")
                        or "No response generated.",
                        confidence=confidence_score,
                        latency_ms=latency_ms,
                    )
                    ...
                    logging.getLogger(__name__).error(
                        f"Failed to save QueryHistory in stream: {db_e}"
                    )
                    db.rollback()
