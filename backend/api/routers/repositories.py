...
def create_repository(
    *,
    ...
    from backend.database.models.models import Repository as DBRepository
    ...
    remote_hash = git_service.get_remote_commit_hash(
        repository_in.git_url, repository_in.default_branch or "main"
    )
    if remote_hash:
        existing_repo = (
            db.query(DBRepository)
            .filter(DBRepository.git_url == repository_in.git_url)
            .first()
        )
        if existing_repo:
            latest_snapshot = (
                db.query(RepositorySnapshot)
                .filter_by(repository_id=existing_repo.repository_id, is_latest=True)
                .first()
            )
    ...
    repo_path = git_service.clone_repository(
        repository_in.git_url, branch=repository_in.default_branch or "main"
    )
...
def read_repository(*, db: Session = Depends(get_db), id: UUID) -> Any:
    """
...
def update_repository(
    *, db: Session = Depends(get_db), id: UUID, repository_in: RepositoryUpdate
) -> Any:
    """
    Update a repository.
    """
...
def delete_repository(*, db: Session = Depends(get_db), id: UUID) -> Any:
    """
    Delete a repository.
    """
    repo = repository_repo.get(db=db, id=id)
...
def list_snapshots(*, db: Session = Depends(get_db), id: UUID) -> Any:
    """
    ...
    snapshots = (
        db.query(RepositorySnapshot)
        .filter(RepositorySnapshot.repository_id == id)
...
def get_latest_snapshot(*, db: Session = Depends(get_db), id: UUID) -> Any:
    """
    ...
    if not snap:
        raise HTTPException(
            status_code=404,
            detail="No snapshot found for this repository. Please trigger ingestion first.",
        )
    return {
