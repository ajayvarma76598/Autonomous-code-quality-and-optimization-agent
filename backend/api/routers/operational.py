...
def get_metrics(db: Session = Depends(get_db)) -> Any:
    ...
    from backend.database.models.models import EvaluationResult, EvaluationRun, QueryHistory
    
    try:
        # --- SUMMARY METRICS ---
        total_evals = db.query(func.count(EvaluationResult.result_id)).scalar() or 0
        passed_evals = db.query(func.count(EvaluationResult.result_id)).filter(EvaluationResult.passed == True).scalar() or 0
        tsr = (passed_evals / total_evals * 100) if total_evals > 0 else 0.0
        
        avg_faithfulness = db.query(func.avg(EvaluationResult.faithfulness)).scalar() or 0.0
        ...
        avg_relevancy = db.query(func.avg(EvaluationResult.answer_relevancy)).scalar() or 0.0
        avg_context_precision = db.query(func.avg(EvaluationResult.context_precision)).scalar() or 0.0
        avg_recall = db.query(func.avg(EvaluationResult.context_recall)).scalar() or 0.0
        avg_llm_confidence = db.query(func.avg(EvaluationResult.llm_confidence)).scalar() or 0.0
        ...
        ).group_by(cast(QueryHistory.created_at, Date)).all()
        ...
        ).join(EvaluationRun, EvaluationResult.evaluation_id == EvaluationRun.evaluation_id)\
         .group_by(cast(EvaluationRun.started_at, Date)).all()
        ...
        for e in eval_stats:
            ...
            day_tsr = (int(e.passed_evals) / int(e.total_evals) * 100) if e.total_evals and e.passed_evals else 0.0
            timeseries_dict[date_str]["tsr"] = round(day_tsr, 2)
            timeseries_dict[date_str]["faithfulness"] = round(float(e.avg_faithfulness or 0), 4)
            timeseries_dict[date_str]["relevancy"] = round(float(e.avg_relevancy or 0), 4)
...
def get_repository_explorer(
    ...
    latest_snapshot = db.query(RepositorySnapshot).filter_by(
        repository_id=id, is_latest=True
    ).first()
    
    if not latest_snapshot:
        return {"error": "No ingested snapshot found for this repository."}
        
    files = db.query(RepositoryFile).filter_by(snapshot_id=latest_snapshot.snapshot_id).all()
