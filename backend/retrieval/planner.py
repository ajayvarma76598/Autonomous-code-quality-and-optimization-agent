...
class RetrievalIntent(BaseModel):
    ...
    search_type: str = Field(description="One of: 'ast', 'dependency', 'semantic', 'exact'.")
    target_modules: List[str] = Field(default_factory=list, description="Specific modules or directories to restrict the search to.")
    
class RetrievalPlan(BaseModel):
    intents: List[RetrievalIntent] = Field(description="List of targeted retrieval intents.")
...
class QueryPlanner:
    ...
    def plan(self, user_question: str, context: str = "") -> List[RetrievalIntent]:
        ...
        """
        try:
            structured_llm = self.llm.with_structured_output(RetrievalPlan, method="function_calling")
        ...
        search_type = "ast" if ("class" in q_lower or "function" in q_lower) else "dependency" if "dep" in q_lower else "semantic"
        return [
