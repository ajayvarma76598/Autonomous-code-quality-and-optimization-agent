...
class VectorStore:
    def retrieve(
        ...
        """
        distance_expr = Embedding.embedding.cosine_distance(query_embedding).label("distance")
        ...
        if snapshot_id:
            try:
                ...
                snap_uuid = UUID(str(snapshot_id)) if isinstance(snapshot_id, str) else snapshot_id
        ...
        stmt = stmt.order_by(distance_expr).limit(top_k * 2)  # over-fetch for re-ranking
        ...
        for emb, chunk, distance in results:
            ...
            retrieved.append({
                ...
                "metadata": {
                    ...
                    "content_type": ct,
                    "start_line":   meta.get("start_line") or getattr(chunk, "start_line", None),
                    "end_line":     meta.get("end_line") or getattr(chunk, "end_line", None),
                    **meta,
                },
            })
