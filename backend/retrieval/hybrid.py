...
def _expand_query(query: str, content_type: Optional[str]) -> str:
    """Expand query with domain vocabulary for better BM25 recall."""
    parts = [query]
    q_lower = query.lower()

    if content_type == "architecture" or "architecture" in q_lower or "summary" in q_lower:
        parts.append("README architecture design ADR microservice overview")
    if content_type == "api" or any(w in q_lower for w in ("api", "endpoint", "controller")):
...
_EXTENSION_BOOST: Dict[str, float] = {
    ".java": 0.60, ".py": 0.60, ".ts": 0.55, ".kt": 0.55,
    ".go":   0.55, ".cs": 0.55, ".js": 0.50, ".sql": 0.45,
    ".md":   0.20,
}
...
class HybridRetriever:
    ...
    def retrieve(
        ...
        if vector_results:
            vector_results = self._expand_with_graph(db, vector_results, query_embedding,
                                                     snapshot_id, top_k)
    ...
    def _rrf_fuse(
        ...
        for rank, doc in enumerate(vector_results or []):
            ...
            if doc_id not in doc_map:
                doc_map[doc_id] = doc
            raw = float(doc.get("vector_score") or doc.get("score") or (1.0 / (rank + 1)))
        ...
        for doc_id, rrf_val in sorted_docs[:top_k]:
            ...
            if any(d in clean for d in [
                "backend/agents/", "backend/retrieval/", "backend/tools/",
                "backend/workflows/", "backend/services/",
            ]):
                continue
