...
class BuiltChunk:
    ...
    parent_symbol: str | None = None  # Parent class name (for method chunks)
...
class ChunkBuilder:
    ...
    def build(
        self, nodes: list[CodeNode], summaries: list[str] | None = None
    ) -> list[BuiltChunk]:
        """
        ...
        if summaries and len(summaries) != len(nodes):
            logger.warning(
                "[ChunkBuilder] Summary list length mismatch — ignoring summaries"
            )
            summaries = None
        ...
        logger.debug(
            f"[ChunkBuilder] Built {len(chunks)} chunks from {len(nodes)} nodes"
        )
        return chunks
    ...
    def _build_node_chunks(
        self, node: CodeNode, summary_text: str | None = None
    ) -> list[BuiltChunk]:
        chunks: list[BuiltChunk] = []
    ...
    def _build_metadata_content(node: CodeNode, meta: dict[str, Any]) -> str:
        ...
        payload = {
            ...
            else (node.parent_name or ""),
            "method": node.name if node.node_type == "function" else "",
            "type": node.node_type,
            "file": node.file_path,
            "imports": [
                imp.strip() for imp in node.imports.splitlines() if imp.strip()
            ][:10],
