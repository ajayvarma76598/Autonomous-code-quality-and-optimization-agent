...
class BM25Retriever:
    ...
    def fit_from_db(self, db, snapshot_id=None):
        ...
        query = db.query(DocumentChunk, RepositoryFile).join(RepositoryFile, DocumentChunk.file_id == RepositoryFile.file_id)
        if snapshot_id:
    ...
    def _auto_fit_workspace(self):
        ...
        if os.path.exists(temp_dir):
            for entry in os.listdir(temp_dir):
                if entry.startswith("repo_") and os.path.isdir(os.path.join(temp_dir, entry)):
                    target_path = os.path.join(temp_dir, entry)
                    for root, dirs, files in os.walk(target_path):
                        # Prune git and build folders
                        dirs[:] = [d for d in dirs if d not in [".git", "node_modules", "target", "build", "dist", "__pycache__"]]
                        for f in files:
                            if f.endswith((".java", ".py", ".ts", ".js", ".go", ".cs", ".sql", ".md", ".json", ".xml", ".yml")):
                                file_path = os.path.join(root, f)
                                rel_path = os.path.relpath(file_path, target_path).replace("\\", "/")
                                try:
