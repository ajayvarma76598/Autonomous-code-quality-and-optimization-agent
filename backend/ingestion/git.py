...
class GitService:
    ...
    def clone_repository(self, git_url: str, branch: str = "main") -> str:
        ...
        if len(parts) <= 4 and ("github.com" in clean_url or "gitlab.com" in clean_url):
            logger.warning(f"URL '{git_url}' appears to be an organization or user profile URL, not a specific repository URL.")
            raise ValueError(f"URL '{git_url}' is an organization/user URL. Please provide a full repository URL (e.g., https://github.com/Wholesome-Care/repo-name)")
        ...
        try:
            ...
            logger.warning(f"Git clone failed for {clean_url}: {e.stderr.strip() if e.stderr else e}")
            raise RuntimeError(f"Failed to clone repository '{clean_url}': {e.stderr.strip() if e.stderr else 'Repository not found'}")
    ...
    def get_latest_commit(self, repo_path: str) -> Dict[str, str]:
        ...
        if not repo_path or not os.path.exists(repo_path) or not os.path.isdir(repo_path):
            logger.warning(f"Directory invalid or not found for git commit check: '{repo_path}'")
            return {
