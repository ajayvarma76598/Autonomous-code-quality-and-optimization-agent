...
class CodeParser:
    ...
    def parse_file(self, file_path: str, language: str = "python") -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            def walk(node):
                ...
                is_definition = any(suffix in node_type for suffix in ["_definition", "_declaration", "_spec", "method_definition", "arrow_function"])
                ...
                if (is_function or is_class) and is_definition:
                    ...
                    for child in node.children:
                        if child.type == "identifier" or child.type == "property_identifier" or child.type == "type_identifier":
                            name = content_bytes[child.start_byte:child.end_byte].decode("utf8")
                            break
                    
                    obj_type = "class" if is_class else "function"
                    
                    # Extract the raw signature (up to the first block)
                    signature = content_bytes[node.start_byte:node.end_byte].decode("utf8").split('\n')[0].strip()
                    if '{' in signature:
    ...
    def _calculate_complexity(self, node) -> int:
        ...
        def walk(n):
            ...
            if node_type in ["if_statement", "for_statement", "while_statement", "catch_clause", "elif_clause", "case_statement", "&&", "||"]:
                complexity += 1
