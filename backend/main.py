...
from backend.api.middleware import TimingAndLoggingMiddleware
from backend.api.routers import repositories, sessions, executions, query, operational, ingestion
...
def create_app() -> FastAPI:
    ...
    app.include_router(repositories.router, prefix=settings.API_V1_STR, dependencies=[Depends(get_current_user)])
    app.include_router(sessions.router, prefix=settings.API_V1_STR, dependencies=[Depends(get_current_user)])
    app.include_router(executions.router, prefix=settings.API_V1_STR, dependencies=[Depends(get_current_user)])
    app.include_router(query.router, prefix=settings.API_V1_STR, dependencies=[Depends(get_current_user)])
    app.include_router(ingestion.router, prefix=settings.API_V1_STR, dependencies=[Depends(get_current_user)])
    
    # Manager/Admin only routes
    app.include_router(operational.router, prefix=settings.API_V1_STR, dependencies=[Depends(require_role(["admin"]))])
    from backend.api.routers import escalation
    app.include_router(escalation.router, prefix=settings.API_V1_STR)
    
    # Register Exception Handlers
    from backend.api.error_handlers import global_exception_handler, database_exception_handler
