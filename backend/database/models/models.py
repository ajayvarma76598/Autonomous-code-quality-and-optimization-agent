...
class Session(Base):
    ...
    repository_id = Column(UUID(as_uuid=True), ForeignKey('repositories.repository_id', ondelete='CASCADE'))
...
class Repository(Base):
    ...
    performance_logs = relationship("PerformanceLog", back_populates="repository", cascade="all, delete-orphan")
...
class RepositorySnapshot(Base):
    ...
    metadata_ = relationship("RepositoryMetadata", back_populates="snapshot", uselist=False, cascade="all, delete-orphan")
    files = relationship("RepositoryFile", back_populates="snapshot", cascade="all, delete-orphan")
    dependency_relationships = relationship("DependencyRelationship", back_populates="snapshot", cascade="all, delete-orphan")
    workflow_runs = relationship("WorkflowRun", back_populates="snapshot", cascade="all, delete-orphan")


class RepositoryFile(Base):
    __tablename__ = 'repository_files'
    file_id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    snapshot_id = Column(UUID(as_uuid=True), ForeignKey('repository_snapshots.snapshot_id', ondelete='CASCADE'))
    path = Column(Text)
    ...
    quality_metrics = relationship("CodeQualityMetric", back_populates="file", cascade="all, delete-orphan")
...
class CodeObject(Base):
    ...
    file_id = Column(UUID(as_uuid=True), ForeignKey('repository_files.file_id', ondelete='CASCADE'))
    parent_object_id = Column(UUID(as_uuid=True), ForeignKey('code_objects.object_id', ondelete='CASCADE'))
    ...
    children = relationship("CodeObject", back_populates="parent", cascade="all, delete-orphan")
    parent = relationship("CodeObject", back_populates="children", remote_side=[object_id])
    dependencies_as_source = relationship("DependencyRelationship", foreign_keys="[DependencyRelationship.source_object_id]", back_populates="source_object")
    dependencies_as_target = relationship("DependencyRelationship", foreign_keys="[DependencyRelationship.target_object_id]", back_populates="target_object")
    document_chunks = relationship("DocumentChunk", back_populates="object", cascade="all, delete-orphan")
...
class DependencyRelationship(Base):
    ...
    snapshot_id = Column(UUID(as_uuid=True), ForeignKey('repository_snapshots.snapshot_id', ondelete='CASCADE'))
    source_object_id = Column(UUID(as_uuid=True), ForeignKey('code_objects.object_id', ondelete='CASCADE'))
    target_object_id = Column(UUID(as_uuid=True), ForeignKey('code_objects.object_id', ondelete='CASCADE'))
    relationship_type = Column(String(50))
    metadata_ = Column("metadata", JSONB)

    snapshot = relationship("RepositorySnapshot", back_populates="dependency_relationships")
    source_object = relationship("CodeObject", foreign_keys=[source_object_id], back_populates="dependencies_as_source")
    target_object = relationship("CodeObject", foreign_keys=[target_object_id], back_populates="dependencies_as_target")
...
class DocumentChunk(Base):
    ...
    file_id = Column(UUID(as_uuid=True), ForeignKey('repository_files.file_id', ondelete='CASCADE'))
    object_id = Column(UUID(as_uuid=True), ForeignKey('code_objects.object_id', ondelete='CASCADE'))
    ...
    retrieval_logs = relationship("RetrievalLog", back_populates="chunk", cascade="all, delete-orphan")
...
class Embedding(Base):
    ...
    chunk_id = Column(UUID(as_uuid=True), ForeignKey('document_chunks.chunk_id', ondelete='CASCADE'))
...
class RetrievalLog(Base):
    ...
    query_id = Column(UUID(as_uuid=True), ForeignKey('query_history.query_id', ondelete='CASCADE'))
    chunk_id = Column(UUID(as_uuid=True), ForeignKey('document_chunks.chunk_id', ondelete='CASCADE'))
...
class WorkflowRun(Base):
    ...
    query_history = relationship("QueryHistory", back_populates="workflow", cascade="all, delete-orphan")
    evaluation_runs = relationship("EvaluationRun", back_populates="workflow", cascade="all, delete-orphan")
...
class QueryHistory(Base):
    ...
    session_id = Column(UUID(as_uuid=True), ForeignKey('sessions.session_id', ondelete='CASCADE'))
    workflow_id = Column(UUID(as_uuid=True), ForeignKey('workflow_runs.workflow_id', ondelete='CASCADE'))
    ...
    retrieval_logs = relationship("RetrievalLog", back_populates="query", cascade="all, delete-orphan")
...
class EvaluationResult(Base):
    ...
    evaluation_id = Column(UUID(as_uuid=True), ForeignKey('evaluation_runs.evaluation_id', ondelete='CASCADE'))
