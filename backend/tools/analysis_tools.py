...
def get_latest_project_key() -> str:
    ...
    db = SessionLocal()
    try:
        latest = db.query(RepositorySnapshot).filter_by(is_latest=True).order_by(RepositorySnapshot.indexed_at.desc()).first()
        if latest:
...
def analyze_code_quality(file_path: str) -> Dict[str, Any]:
    ...
    if not project_key:
        return {"error": "No ingested repository found in the database to determine the SonarQube project key."}
    ...
    try:
        ...
        if response.status_code == 404:
            return {"error": f"File {clean_path} not found in SonarQube. Ensure the file exists and was scanned during ingestion."}
