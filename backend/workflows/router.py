...
from backend.agents.manager.manager_agent import (
    ManagerAgent,  # This acts as our Intent Router now
)
...
def human_validation_node(state: AgentState) -> AgentState:
    ...
    """
    logger.info("Human validation completed.")
...
class WorkflowRouter:
    ...
    def _build_graph(self):
        ...
        workflow.add_conditional_edges("repository", route_after_repository)
        ...
        workflow.add_conditional_edges(
            ...
            {"human_validation": "human_validation", "FINISH": END},
        )

        # Human validation terminates the graph
        # (or we could route back if we wanted a retry, but we terminate to prevent loops)
        workflow.add_edge("human_validation", END)

        # Compile with a checkpointer and interrupt
        return workflow.compile(
            checkpointer=self.memory, interrupt_before=["human_validation"]
        )
    ...
    def invoke(self, state: dict, config: dict = None) -> dict:
        ...
        from backend.workflows.intent_classifier import (
            ComplexityLevel,
            intent_classifier,
        )
        ...
        state["workflow"]["execution_plan"] = {
            ...
            if level in (ComplexityLevel.L1_RETRIEVAL, ComplexityLevel.L2_SINGLE_AGENT)
            else "multi_agent",
            "agents": workers,
        }

        logger.info(
            f"[WorkflowRouter] Invoking query (Level: {level.value} "
            f"| Intent MS: {ic_ms}ms | Workers: {workers})"
        )

        invoke_config = config or {}
        if "configurable" not in invoke_config:
            invoke_config["configurable"] = {}
        invoke_config["configurable"]["thread_id"] = state.get("shared", {}).get(
            ...
            for agent_name, agent_state in event.items():
                ...
                if "messages" in agent_state and agent_state["messages"]:
                    messages_to_log = (
                        ...
                        if isinstance(agent_state["messages"], list)
                    ...
                    for msg in messages_to_log:
                        ...
                        if msg_type == "tool":
                            ...
                            logger.info(
                                f"[{agent_name} Called Tools] -> {tools_called}"
                            )
                            if msg.content:
                                logger.info(
                    ...
                    if last_msg:
                        session_id = state.get("shared", {}).get(
                            "session_id", "unknown"
                        )
        ...
        det_eval = deterministic_validator.validate_output(
            final_resp, citations, shared_ctx
        )
        ...
        try:
            persistence_service.flush_transaction(
