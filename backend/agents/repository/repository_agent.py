...
class RepositoryAgent(BaseAgent):
    ...
    def execute(self, state: AgentState) -> AgentState:
        ...
        repo_path_or_url = shared.get("repo_path_or_url") or shared.get(
            "repository_path"
        )
        ...
        try:
            ...
            if snapshot_id:
                snap = (
                    ...
                    .filter(RepositorySnapshot.snapshot_id == snapshot_id)
                    .first()
                )
        ...
        if not repo_path_or_url:
            ...
            if query_val and (
                query_val.startswith("http")
                or query_val.startswith("git@")
                or (os.path.exists(query_val) and os.path.isdir(query_val))
            ):
        ...
        try:
            ...
            persistence_service.queue_sonar_metrics(
                analysis_id, "global", sonar_context
            )
            ...
            try:
                ...
                deps = (
                    [
                        {
                            "source": str(d.source_object_id),
                            "target": str(d.target_object_id),
                            "type": d.relationship_type or "import",
                        }
                        for d in db_deps
                    ]
                    if db_deps
            ...
            state = StateManager.append_message(
                state, AIMessage(content=f"Repository ingestion failed: {e}")
            )
