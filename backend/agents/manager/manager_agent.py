...
class ManagerAgent(BaseAgent):
    ...
    def execute(self, state: AgentState) -> AgentState:
        ...
        """
        logger.info(f"Manager evaluating state for session {state.get('shared', {}).get('session_id', 'unknown')}")
        ...
        try:
            ...
            class ExecutionPlan(BaseModel):
                workflow_type: WorkflowType = Field(description="The core workflow to execute.")
                agents: List[str] = Field(description="List of specific agents that will be executed.")
                parallel: bool = Field(default=False, description="True if the agents can be executed in parallel.")
                dependencies: List[str] = Field(default_factory=list, description="Any strict dependencies required before execution.")
            ...
            prompt = ChatPromptTemplate.from_messages([
                ("system", "You are an expert routing supervisor. Classify the user query into the appropriate workflow_type and set the exact list of target agents.\n\n"
            ...
            plan_dict["workflow_type"] = result.workflow_type.value if hasattr(result.workflow_type, "value") else str(result.workflow_type)
            state["workflow"]["execution_plan"] = plan_dict
            state["workflow"]["workflow_type"] = result.workflow_type
            state["workflow"]["next_node"] = result.workflow_type.value if hasattr(result.workflow_type, "value") else str(result.workflow_type)
        ...
        logger.info(f"Manager created execution plan: {state.get('workflow', {}).get('execution_plan')}")
