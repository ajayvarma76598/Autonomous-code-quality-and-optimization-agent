...
class EvaluationFailure(BaseModel):
    category: Literal["Accuracy", "Security", "Completeness", "Formatting"] = Field(
    ...
    suggested_fix: str = Field(
        description="A targeted suggestion on how to fix this specific failure without regenerating the entire response."
    )


class ClaimEvidenceValidation(BaseModel):
    claim: str = Field(
        description="The specific factual statement or finding in the output."
    )
    evidence_state: Literal["STRONG", "PARTIAL", "MISSING"] = Field(
        description="STRONG if backed by source code/AST/Sonar, PARTIAL if backed by docs, MISSING if unsupported."
    )
    evidence_type: Literal[
        "SOURCE_CODE", "STATIC_ANALYSIS", "SQL_METADATA", "DOCUMENTATION", "INFERRED"
    ] = Field(description="The provenance tier of the supporting evidence.")
    confidence: float = Field(
        description="Calibrated confidence score (0.0 to 1.0) based on evidence provenance."
    )
...
class EvaluationResult(BaseModel):
    verdict: Literal["PASS", "RETRY", "FAIL"] = Field(
        description="PASS if acceptable, RETRY if fixable, FAIL if fundamentally broken or ungrounded."
    )
    critique: str = Field(
        default="", description="Detailed qualitative feedback explaining the verdict."
    )
    confidence_score: float = Field(
        description="Calibrated aggregate confidence score from 0.0 to 1.0."
    )
    total_claims: int = Field(
        default=1, description="Total number of factual statements in the output."
    )
    supported_claims: int = Field(
        default=1,
        description="Number of factual statements backed by STRONG or PARTIAL evidence.",
    )
    claim_faithfulness_score: float = Field(
        default=1.0, description="Calculated ratio: supported_claims / total_claims."
    )
    claim_validations: list[ClaimEvidenceValidation] = Field(
        default_factory=list, description="Per-claim 3-state evidence validations."
    )
    failures: list[EvaluationFailure] = Field(
        default_factory=list,
        description="List of specific failures if the verdict is not PASS.",
    )
...
class EvaluationAgent(BaseAgent):
    ...
    def execute(self, state: AgentState) -> AgentState:
        ...
        if isinstance(existing_eval, dict) and not existing_eval.get(
            "requires_llm_eval", True
        ):
            logger.info(
                f"[EvaluationAgent] [Tier 1 Optimization] Deterministic Validation passed (Confidence: {existing_eval.get('confidence', 0.90)}). Skipping Tier 2 LLM evaluation."
            )
            return state
        ...
        eval_template = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are an impartial Quality Gate and Anti-Hallucination Evaluator.\n"
        ...
        messages = eval_template.format_messages(question=query, draft_answer=draft)
        ...
        ).with_config({"run_name": "EvaluationAgent"})
        result = structured_llm.invoke(messages)

        eval_duration = time.time() - start_time
        logger.info(
            f"Evaluation took {eval_duration:.2f}s. Verdict: {result.verdict} (Confidence: {result.confidence_score})"
        )
        ...
        faithfulness = (
            round(result.claim_faithfulness_score, 4)
            if result.total_claims > 0
            else round(result.confidence_score, 4)
        )
        ...
        strong_claims = sum(
            1 for c in result.claim_validations if c.evidence_state == "STRONG"
        )
        recall = (
            round(strong_claims / result.total_claims, 4)
            if result.total_claims > 0
            else 0.0
        )
        # Context precision: fraction of STRONG claims out of all non-missing
        non_missing = sum(
            1 for c in result.claim_validations if c.evidence_state != "MISSING"
        )
        context_precision = (
            round(strong_claims / non_missing, 4) if non_missing > 0 else 0.0
        )
