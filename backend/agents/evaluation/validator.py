...
class DeterministicValidator:
    ...
    """
    def validate_output(self, response_text: str, citations: List[Any], context_chunks: List[Any]) -> Dict[str, Any]:
        """
        ...
        file_path_matches = re.findall(r"[\w\/\.\-]+\.(?:py|java|js|ts|json|md|yml|yaml|xml)", response_text, re.IGNORECASE)
        repository_matches = len(file_path_matches) > 0 or cited_count > 0
        
        # 4. Metrics Verified?
        metrics_present = bool(re.search(r"\b\d+(?:\.\d+)?%\b|\b\d+\s*ms\b", response_text))
        metrics_verified = True if not metrics_present else (cited_count > 0 or required_evidence_present)
        
        # 5. Coverage Calculated
        coverage_calculated = round(min(1.0, cited_count / max(1, min(5, total_chunks))), 4) if total_chunks > 0 else (1.0 if cited_count > 0 else 0.5)
        
        # 6. Confidence Computed
        faithfulness = round(min(1.0, 0.70 + (coverage_calculated * 0.30)), 4) if citation_exists or repository_matches else 0.60
        ...
        logger.info(f"[DeterministicValidator Tier 1] Citations: {citation_exists} | Repo Match: {repository_matches} | Evidence: {required_evidence_present} | Coverage: {coverage_calculated:.2f} | Confidence: {confidence} | Requires LLM: {requires_llm}")
        ...
        return {
            ...
            "reason": "Tier 1 deterministic validation passed" if not requires_llm else "Tier 1 confidence below threshold, triggering Tier 2 LLM validation"
        }
