...
class CoverageFinding(BaseAnalysisFinding):
    tested_status: Literal["FULLY_TESTED", "PARTIALLY_TESTED", "UNTESTED"] = Field(description="The test coverage status of this specific module/file.")
    risk_level: Literal["CRITICAL", "HIGH", "MEDIUM", "LOW"] = Field(description="The risk level of this missing coverage, influenced by architectural importance.")
    file_path: Optional[str] = Field(default=None, description="The file path lacking coverage.")
    suggested_test_cases: Optional[List[str]] = Field(default=None, description="A descriptive list of specific test cases that should be written to cover the missing lines (e.g., 'Write a test for the null user edge case').")

class CoverageResult(BaseAnalysisResult):
    findings: List[CoverageFinding] = Field(description="List of structured test coverage findings.")
...
class TestCoverageAgent(BaseAgent):
    ...
    def execute(self, state: AgentState) -> AgentState:
        ...
        evidence_context = evidence_service.gather_evidence("coverage", repository_id, analysis_id, query=query)
            ...
            if sonar_host and sonar_token:
                ...
                if resp.status_code == 200:
                    ...
                    for c in components:
                        ...
                        if uncov > 0:
                            worst_files.append({"path": c.get("path"), "uncovered": uncov})
                                ...
                                if os.path.exists(full_file_path):
                                    try:
                                        with open(full_file_path, "r", encoding="utf-8") as f:
                                            ...
                                            snippet = "".join([f"{idx+1}: {snippet_lines[idx]}" for idx in range(len(snippet_lines))])
                                            if len(lines) > 100:
                                                snippet += "\n... (file truncated) ..."
                                    except Exception:
                                        pass
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are an expert Test Engineer. Review the Evidence Context, Architecture Findings, and Uncovered Files.\n"
        ...
        structured_llm = self.llm.with_structured_output(CoverageResult, method="function_calling")
        result: CoverageResult = structured_llm.invoke(prompt.format_messages(
