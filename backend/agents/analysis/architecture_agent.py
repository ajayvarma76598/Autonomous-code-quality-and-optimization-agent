...
from backend.tools.parsing_tools import ingest_document
...
class ArchitectureFinding(BaseAnalysisFinding):
    issue_type: Literal["FACT", "OPINION", "SUGGESTION"] = Field(description="Whether this is an objective fact, an opinion on design, or a suggestion.")

class ArchitectureResult(BaseAnalysisResult):
    findings: List[ArchitectureFinding] = Field(description="List of structured architectural findings.")
...
class ArchitectureAgent(BaseAgent):
    ...
    def execute(self, state: AgentState) -> AgentState:
        ...
        evidence_context = evidence_service.gather_evidence("architecture", repository_id, analysis_id, query=query)
        
        if not evidence_context:
            logger.error("ArchitectureAgent: Missing repository_context in shared state.")
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are an Expert Software Architect reviewing a repository.\n\n"
        ...
        structured_llm = self.llm.with_structured_output(ArchitectureResult, method="function_calling")
        result: ArchitectureResult = structured_llm.invoke(prompt.format_messages(evidence_context=evidence_context.format_for_prompt()))
        ...
        if evidence_context.dependency_graph:
            analysis_payload["dependency_graph"] = evidence_context.dependency_graph.model_dump() if hasattr(evidence_context.dependency_graph, "model_dump") else evidence_context.dependency_graph.dict()
