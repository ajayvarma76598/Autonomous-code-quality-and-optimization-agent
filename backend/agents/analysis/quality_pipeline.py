...
def repository_parser_node(state: AgentState) -> dict:
    ...
    logger.info(
        f"Pipeline Node: repository_parser_node for session {state.get('shared', {}).get('session_id', 'unknown')}"
    )
    ...
    structured_llm = llm.with_structured_output(
        ExtractedFiles, method="function_calling"
    )
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "Extract any specific file paths or names mentioned in the user query. If none, return an empty list.",
            ),
            ("human", "{query}"),
        ]
    )
...
def sonarqube_metrics_node(state: AgentState) -> dict:
    ...
    logger.info(
        f"Pipeline Node: sonarqube_metrics_node for session {state.get('shared', {}).get('session_id', 'unknown')}"
    )
                ...
                if measures:
                    ...
                    try:
                        issues_resp = requests.get(
                            issues_url, auth=(sonar_token, ""), timeout=10
                        )
                        ...
                        logger.error(
                            f"SonarQube issues API error for {component_key}: {ie}"
                        )
...
def architecture_reviewer_node(state: AgentState) -> dict:
    ...
    logger.info(
        f"Pipeline Node: architecture_reviewer_node for session {state.get('shared', {}).get('session_id', 'unknown')}"
    )
    ...
    for file, code in parsed_code.items():
        ...
        messages = [
            SystemMessage(
                content="You are a strict architecture reviewer. Provide concise feedback."
            ),
...
def report_generator_node(state: AgentState) -> dict:
    ...
    logger.info(
        f"Pipeline Node: report_generator_node for session {state.get('shared', {}).get('session_id', 'unknown')}"
    )
