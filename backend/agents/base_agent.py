...
from backend.agents.executor import LLMInvocationError, ToolExecutor
...
class BaseAgent:
    ...
    def get_system_prompt(self) -> str:
        ...
        logger.warning(
            f"Failed to fetch {self.name}_prompt from Langfuse. Using local fallback."
        )
    ...
    def execute_with_tools(self, state: AgentState, tools: list) -> AgentState:
        ...
        llm_with_tools = self.llm.bind_tools(tools).with_config(
        ...
        if state["shared"].get("query") and not any(
            getattr(m, "content", "") == state["shared"]["query"] for m in messages
        ):
        ...
        logger.info(
            f"[AGENT REQUEST] [{self.full_name}] -> Query: '{user_query}' | Session: {session_id}"
        )
        ...
        try:
            ...
            result_msg = ToolExecutor.execute(
                llm_with_tools, tools, messages, state, plain_llm=self.llm
            )
            ...
            if self.output_schema:
                ...
                if not resp_content or not str(resp_content).strip():
                    ...
                    for m in reversed(messages):
                        if (
                            (isinstance(m, AIMessage) or getattr(m, "type", "") == "ai")
            ...
            logger.info(
                f"[AGENT RESPONSE] [{self.name}] -> Output Length: {len(str(resp_content))} chars | Snippet: {str(resp_content)[:120]}..."
            )
