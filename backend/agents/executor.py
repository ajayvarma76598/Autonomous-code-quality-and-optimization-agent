...
class ToolExecutor:
    ...
    @staticmethod
    def execute(llm_with_tools, tools: list, messages: list, state: AgentState, max_iterations: int = 5, plain_llm=None) -> Any:
        """
        ...
        try:
            ...
            while iteration < max_iterations:
                iteration += 1
                logger.info(f"[LLM REQUEST] [{agent_name}] (Iteration {iteration}) -> Messages count: {len(messages)}")
                
                llm_start = time.time()
                result_msg = llm_with_tools.invoke(messages)
                llm_duration = time.time() - llm_start
                tool_calls = getattr(result_msg, "tool_calls", []) or []
                logger.info(f"[LLM RESPONSE] [{agent_name}] -> Took {llm_duration:.2f}s | Tool calls count: {len(tool_calls)}")
                ...
                for tool_call in tool_calls:
                    ...
                    logger.info(f"   [TOOL REQUEST] [{agent_name} -> Tool: {t_name}] | Arguments: {t_args}")
                    ...
                    logger.info(f"   [TOOL RESPONSE] [{t_name}] -> Output Length: {len(res_str)} chars | Snippet: {res_str[:120]}...")
                    messages.append(ToolMessage(content=res_str, tool_call_id=tool_call["id"]))
                
                tool_duration = time.time() - tool_start
                logger.info(f"[TOOL LAYER COMPLETED] [{agent_name}] (Iteration {iteration}) -> Took {tool_duration:.2f}s")
            ...
            if not content or not str(content).strip():
                logger.info(f"[LLM SYNTHESIS FORCED] [{agent_name}] -> Synthesizing tool outputs into final response text...")
                target_llm = plain_llm if plain_llm else llm_with_tools
                synthesis_start = time.time()
                result_msg = target_llm.invoke(messages)
                synthesis_duration = time.time() - synthesis_start
                logger.info(f"[LLM SYNTHESIS COMPLETED] [{agent_name}] -> Took {synthesis_duration:.2f}s | Final Content Length: {len(str(getattr(result_msg, 'content', '')))}")
                messages.append(result_msg)

            total_duration = time.time() - start_time
            logger.info(f"[EXECUTOR FINISHED] [{agent_name}] -> Total agent execution took {total_duration:.2f}s")
