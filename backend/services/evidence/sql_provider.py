...
class SQLProvider(BaseEvidenceProvider):
    ...
    def fetch(
        ...
        try:
            ...
            try:
                ...
                if query:
                    ...
                    for kw in keywords[:3]:
                        ...
                        objs = (
                            db.query(CodeObject)
                            .filter(CodeObject.name.ilike(f"%{kw}%"))
                            .limit(10)
                            .all()
                        )
                        for o in objs:
                ...
                dep_count = (
                    db.query(DependencyRelationship)
                    .filter(DependencyRelationship.snapshot_id == snap.snapshot_id)
                    .count()
                )
                meta = (
                ...
                if matched_objects:
                    obj_str = ", ".join(
                        [
                            f"{o['name']} ({o['type'] or 'symbol'}, complexity: {o['complexity'] or 0})"
            ...
            data = (
                f"SQL Database query for '{query}': database session query error ({e})."
            )
