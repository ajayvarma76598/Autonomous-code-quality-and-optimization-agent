        ...
        try:
            ...
            try:
                ...
                if not snap:
                    ...
                    if repository_id:
                        logger.warning(f"No snapshot found for repository {repository_id}. Skipping vector retrieval.")
                        return None
                ...
                chunks = hybrid_retriever.retrieve(db, query, query_embedding, top_k=top_k, snapshot_id=snapshot_id)
                
                # Filter out internal system paths from repository evidence
                clean_chunks = []
                for c in chunks:
                    clean_path = str(c.repository_path).replace("\\", "/")
                    if any(internal_dir in clean_path for internal_dir in ["backend/agents/", "backend/retrieval/", "backend/tools/", "backend/workflows/", "backend/services/"]):
                        continue
                ...
                if clean_chunks:
                    ...
                    return EvidenceBlock(
                        data=clean_chunks,
                        confidence_score=round(float(avg_score * 20.0), 2) if avg_score < 1.0 else 0.85,
