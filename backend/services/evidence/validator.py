...
class EvidenceValidator:
    ...
    def validate(context: EvidenceContext, agent_type: str) -> EvidenceContext:
        ...
        if agent_type in ["coverage", "performance", "quality"]:
            if (
                not context.sonar_metrics
                or not context.sonar_metrics.data
                or "uninitialized" in str(context.sonar_metrics.data)
            ):
                is_complete = False
                warnings.append(
                    f"SonarQube metrics are uninitialized or unavailable for agent '{agent_type}'."
                )
        ...
        if agent_type in ["repository", "architecture", "performance", "documentation"]:
            if not context.retrieved_chunks or not context.retrieved_chunks.data:
                is_sufficient = False
                warnings.append(
                    f"No retrieved code or documentation chunks found for query in agent '{agent_type}'."
                )
