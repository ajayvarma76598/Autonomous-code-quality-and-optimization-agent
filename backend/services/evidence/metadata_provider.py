...
class MetadataProvider(BaseEvidenceProvider):
    ...
    def fetch(self, repository_id: str, analysis_id: Optional[str] = None, **kwargs) -> Optional[EvidenceBlock]:
        try:
            ...
            from backend.database.models.models import RepositoryMetadata, Repository
            ...
            try:
                ...
                language = repo.default_language if repo and repo.default_language else "unknown"
                tech_stack = meta.technology_stack if meta and meta.technology_stack else {}

                data = {
                    "language": language,
                    "technology_stack": tech_stack,
                    "architecture_summary": meta.architecture_summary if meta and meta.architecture_summary else "Architecture summary not yet generated. Please re-ingest the repository.",
                    "entry_points": meta.entry_points if meta and meta.entry_points else []
                }
            finally:
                db.close()
        except Exception as e:
            logger.info(f"MetadataProvider fallback query: {e}")
            data = {"language": "unknown", "framework": "unknown", "architecture": "unknown"}
