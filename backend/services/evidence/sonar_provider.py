        ...
        try:
            ...
            try:
                ...
                if not snap:
                    return None
                if snap:
                    files = db.query(RepositoryFile).filter(RepositoryFile.snapshot_id == snap.snapshot_id).all()
                    file_ids = [f.file_id for f in files]
                    metrics = db.query(CodeQualityMetric).filter(CodeQualityMetric.file_id.in_(file_ids)).all() if file_ids else []
                else:
                    files = []
                    metrics = []
                    
                if metrics:
                    avg_coverage = sum(m.test_coverage_percentage or 0.0 for m in metrics) / len(metrics)
                    total_complexity = sum(m.cyclomatic_complexity or 0.0 for m in metrics)
                    total_smells = sum(m.code_smell_count or 0 for m in metrics)
                    total_vulnerabilities = sum(m.security_vulnerability_count or 0 for m in metrics)
                    total_bugs = sum(getattr(m, "bugs_count", 0) or 0 for m in metrics)
                    total_hotspots = sum(getattr(m, "security_hotspots_count", 0) or 0 for m in metrics)
                    avg_maintainability = sum(m.maintainability_index or 0.0 for m in metrics) / len(metrics)
                    gate_passed = all((m.test_coverage_percentage or 0) >= 80.0 for m in metrics)
