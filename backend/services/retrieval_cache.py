...
class RetrievalCache:
    ...
    def __init__(self):
        ...
        if not redis_host:
            logger.info("REDIS_HOST not set in environment. Retrieval cache using local fallback dict.")
            return
        ...
        try:
            ...
            logger.info(f"[Redis PING] Testing connection to {redis_host}:{redis_port} (db=1)")
            self.redis_client.ping()
            logger.info(f"[Redis PING] Connected successfully to Redis for retrieval cache at {redis_host}:{redis_port} (db=1)")
        except Exception as e:
            logger.warning(f"Failed to connect to Redis for retrieval cache: {e}. Using local fallback dict.")
            self.redis_client = None
    ...
    def get(self, query: str, top_k: int) -> Optional[List[Dict[str, Any]]]:
        ...
        if self.redis_client:
            try:
                logger.info(f"[Redis GET] Fetching key '{key}' (db=1) for query: '{query}'")
    ...
    def set(self, query: str, top_k: int, results: Any):
        ...
        if self.redis_client:
            try:
                ...
                logger.info(f"[Redis SETEX SUCCESS] Key '{key}' saved successfully (db=1)")
            except Exception as e:
