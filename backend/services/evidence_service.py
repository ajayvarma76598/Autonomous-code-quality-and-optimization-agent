...
class EvidenceService(BaseService):
    ...
    def gather_evidence(
        ...
        def _gather() -> EvidenceContext:
            ...
            logger.info(
                f"[EVIDENCE GATHERING STARTED (PARALLEL)] Agent Type: '{agent_type}' | Repo: '{repository_id}' | Analysis: '{analysis_id}' | Top_K: {top_k}"
            )
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
                ...
                if agent_type in ["repository", "documentation"]:
                    futures["metadata"] = executor.submit(
                        metadata_provider.fetch, repository_id, analysis_id
                    )
                ...
                if agent_type in ["coverage", "performance", "quality"]:
                    futures["sonar"] = executor.submit(
                        sonar_provider.fetch, repository_id, analysis_id
                    )

                # 4. Vector & Hybrid Search Provider
                if query and agent_type in [
                    "repository",
                    "architecture",
                    "performance",
                    "documentation",
