...
class PersistenceService(BaseService):
    ...
    def queue_agent_result(
        self, analysis_id: str, agent_name: str, result: Any
    ) -> None:
    ...
    def queue_sonar_metrics(
        self, analysis_id: str, file_id: str, sonar_context: Any
    ) -> None:
        ...
        staging["sonar_metrics"].append({"file_id": file_id, "metrics": metrics_dict})
        logger.info(
            f"Queued Sonar metrics for file {file_id} in analysis {analysis_id}"
        )
    ...
    def queue_dependencies(self, analysis_id: str, dependencies: list[Any]) -> None:
        ...
        logger.info(
            f"Queued {len(dependencies)} dependencies for analysis {analysis_id}"
        )
    ...
    def flush_transaction(
        self, analysis_id: str, repository_id: str, final_status: str = "COMPLETED"
    ) -> bool:
        """
        ...
        def _flush():
            ...
            staging["completed_at"] = datetime.now(UTC)

            logger.info(
                f"--- STARTING ATOMIC DB TRANSACTION FOR ANALYSIS {analysis_id} ---"
            )
            ...
            if staging["sonar_metrics"]:
                logger.info(
                    f"Persisted {len(staging['sonar_metrics'])} CodeQualityMetric records."
                )

            # 4. Persist Dependencies
            if staging["dependency_graph"]:
                logger.info(
                    f"Persisted {len(staging['dependency_graph'])} DependencyRelationship records."
                )
