...
class CacheService:
    def __init__(self):
        ...
        if not redis_host:
            logger.info(
                "REDIS_HOST not set in environment. Redis semantic cache disabled."
            )
            return
        ...
        try:
            ...
            # Test connection
            logger.info(
                f"[Redis PING] Testing connection to {redis_host}:{redis_port} (db=0)"
            )
            self.redis_client.ping()
            logger.info(
                f"[Redis PING] Connected successfully to Redis at {redis_host}:{redis_port} (db=0)"
            )
        except Exception as e:
            logger.warning(
                f"Failed to connect to Redis: {e}. Semantic caching will be disabled."
            )
            self.redis_client = None
        ...
        try:
            ...
            for entry_json in cached_entries:
                ...
                if entry.get("query", "").strip().lower() == q_clean:
                    logger.info(
                        f"[Redis FAST Cache HIT] Sub-1ms exact text match for session {session_id}"
                    )
                    return {
    ...
    def check_cache(
        self, session_id: str, query_text: str, query_embedding: list
    ) -> dict:
        """
        ...
        try:
            ...
            cached_entries = self.redis_client.lrange(cache_key, 0, -1)
            logger.info(
                f"[Redis LRANGE] Retrieved {len(cached_entries)} entries for key '{cache_key}'"
            )
            ...
            for entry_json in cached_entries:
                ...
                if similarity >= 0.95:
                    logger.info(
                        f"[Redis Cache HIT] Similarity: {similarity:.4f} for session {session_id}"
                    )
                    return {
                        "response": entry.get("response", ""),
                        "metadata": entry.get("metadata", {}),
            ...
            logger.info(
                f"[Redis Cache MISS] No match above threshold for session {session_id}"
            )
            return None

        except Exception as e:
            logger.error(
                f"[Redis LRANGE ERROR] Error checking cache for key '{cache_key}': {e}"
            )
            return None
    ...
    def save_to_cache(
        ...
        try:
            ...
            logger.info(
                f"[Redis EXPIRE] Setting 86400s TTL on key '{cache_key}' (db=0)"
            )
            self.redis_client.expire(cache_key, 86400)
            logger.info(
                f"[Redis SAVE SUCCESS] Entry saved and TTL updated for key '{cache_key}'"
            )
        except Exception as e:
