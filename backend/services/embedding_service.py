...
class EmbeddingService:
    ...
    def generate_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        if not self.embeddings:
            logger.warning("[EmbeddingService] Embeddings object uninitialized. Returning zero vectors.")
            return [[0.0] * 1536 for _ in texts]
        if not texts:
            return []
            
        try:
            logger.info(f"[EmbeddingService] Generating embeddings for batch of {len(texts)} texts...")
            res = self.embeddings.embed_documents(texts)
            logger.info(f"[EmbeddingService] Batch embedding generation complete ({len(res)} vectors generated).")
            return res
        except Exception as e:
            logger.warning(f"[EmbeddingService] Error generating document embeddings: {e}")
            return [[0.0] * 1536 for _ in texts]
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        if not self.embeddings or not text:
            return [0.0] * 1536
        try:
            logger.info(f"[EmbeddingService] Embedding single query (len={len(text)})...")
            res = self.embeddings.embed_query(text)
            logger.info(f"[EmbeddingService] Query embedding complete (dim={len(res)}).")
            return res
        except Exception as e:
