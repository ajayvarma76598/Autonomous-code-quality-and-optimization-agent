...
class RepositoryFacade(BaseService):
    ...
    def get_repository_context(
        ...
        def _get_context():
            logger.info(
                f"RepositoryFacade fetching context for {repo_url_or_path} (branch: {branch})"
            )
            ...
            repo_id = (
                str(target_path).split("/")[-1].split("\\")[-1].replace(".git", "")
            )

            local_path = None
            if str(target_path).startswith("http") or str(target_path).startswith(
                "git@"
            ):
                ...
                if possible_temps and os.path.exists(possible_temps[-1]):
                    ...
                    logger.info(
                        f"Using existing local cloned repository at '{local_path}'"
                    )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Git clone failed for '{target_path}': {e}. Searching fallback temp repos."
                        )
                        all_temps = glob.glob(os.path.join(temp_dir, "repo_*"))
                        if all_temps:
                            local_path = all_temps[-1]
                            logger.info(
                                f"Falling back to local repo directory '{local_path}'"
                            )
                        else:
                ...
                if (
                    target_path
                    and os.path.exists(str(target_path))
                    and os.path.isdir(str(target_path))
                ):
