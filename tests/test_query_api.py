...
def mock_dependencies(mocker):
    ...
    mocker.patch('backend.services.cache_service.cache_service.check_exact_cache', return_value=None)
    mocker.patch('backend.services.embedding_service.embedding_service.embed_query', return_value=None)
    
    # Mock langfuse config
    mocker.patch('backend.services.observability.langfuse.langfuse_service.langfuse', new=None)
    
    # Mock the workflow_router get_state
    mock_state_wrapper = mocker.MagicMock()
    mock_state_wrapper.values = {}
    mocker.patch('backend.workflows.router.workflow_router.workflow.get_state', return_value=mock_state_wrapper)
    ...
    mock_workflow_output = {
        ...
        "workflow": {
            ...
            "current_execution_start_indices": {"messages": 0, "context": 0, "execution_path": 0}
        }
    }
    mocker.patch('backend.workflows.router.workflow_router.invoke', return_value=mock_workflow_output)
